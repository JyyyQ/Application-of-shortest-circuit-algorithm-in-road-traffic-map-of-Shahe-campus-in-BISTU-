# -*- coding: utf-8 -*-
"""
Created on Wed Apr 28 18:45:57 2021

@author: hp
"""
import cv2
from floyd import floyd
from floyd import print_path
from floyd import print_single_path
from floyd import turn
...
if __name__=='__main__':
    ...
    listg=turn(g)
    n = len(listg)
    parents = [[i] * n for i in range(len(listg))]  # 关键地方，i-->j 的父结点初始化都为i
    floyd(listg,parents)
    a=input('请输入起点：')
    b=input('请输入终点：')
    z,x=chara_turn_num(a,b,key)
    path_list=[]
    print_single_path(int(z),int(x),path_list,parents)
    print('\n')
   # print(path_list)
    print('最短路径：',end='')
    for i in range(len(path_list)-1):
        print(path_list[i],end='')
        if path_list[i] in key:
            print('('+key[path_list[i]]+')',end='')
        print('->',end='')
    print(path_list[len(path_list)-1],end='')
    print('('+key[path_list[len(path_list)-1]]+')')
    print('最短距离：',end='')
    a=listg[int(z)][int(x)]/2.9
    print(decimal.Decimal(a).quantize(decimal.Decimal('0.01')),'米')
    print('走路预算时间：',end='')
    b=listg[int(z)][int(x)]/2.9*1.1/60
    e=listg[int(z)][int(x)]/2.9*1.1%60
    print(int(b),'分钟',end='')
  ...
  #  img=cv2.resize(img,((1316,903)),interpolation=cv2.INTER_LINEAR)
    ...
    for i in range(len(path_list)):
        if path_list[i] in coordinate:
            w.append(coordinate[path_list[i]])
   # print('最短路的坐标路径：',w)
   ...
   # cv2.namedWindow("image")
    ...
    #cv2.waitKey(0)
    cv2.imwrite("./floyd_route.jpg", img, [int(cv2.IMWRITE_JPEG_QUALITY), 100]) 
    im = Image.open('floyd_route.jpg')
    im.show()
